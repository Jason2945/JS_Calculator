...
function buttonPressed(classValue){
    ...
    switch(classValue){
        ...
        case 'equal' :
            if(symbolOperators.includes(screenText[screenText.length - 1])) {
                currentChar = ""
                break;
            }else {
                currentChar = '=';
                convertToArray(screenText)
                break;
            } 
        }
...
function convertToArray(inputtedString){
    const expression = inputtedString.match(new RegExp(`\\d+|\\${symbolOperators.join('|\\')}`, 'g'));
    let doMath = 0;
...
function solveEquation(expression){
    ...
    console.log("combined sum = ", combinedSum)
}